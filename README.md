# line-art
 ...
 The idea grew into the motivation of capturing and recreating the contour our bodies through processing the different pixel regions (shades) that is cast on us as we interact with light. With precise calculations to deduce the length, color, and location of numerous straight lines, the human body becomes art.  
 ...
 The second file, 'lineArt1', performs similarly to the prior, only that it stored the location of the face from the input image and pastes the final canvas back onto the original image at the same position. This results in the output image having the background (with color) in-tact while the detected face will be replaced by the line-filled replica, drawn on a white canvas.
