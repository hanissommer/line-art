...
class Utils:
    ...
    def face_time(self, frame, faces):
        if len(faces) == 0:
        ...
        for f in faces:
            ...
            # height1, width1 = past_human_detection.shape
            # print(height1, width1)

            #If face_neck takes up a third of the frame, return yes
            if (width1*height1) > 14000:
                if self.big_screen == False:
                    print(f"small width1, small height1", width1, height1)
                    self.cap.release()
                    self.cap = cv2.VideoCapture(1)
                    self.choose_best_resolution()
                    self.big_screen = True
                    self.res_change = True
            elif (width1*height1) < 64000 and self.big_screen == True:
                print(f"big width1, big height1", width1, height1)
                self.cap.release()
                self.cap = cv2.VideoCapture(1)
                self.cap.set(cv2.CAP_PROP_FRAME_WIDTH, 640)
                self.cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 480)
